...
function findElementGreaterThan(arr, num) {
  if (Array.isArray(arr)) {
    const foundElement = arr.find((max) => max > num);
    return foundElement;
  }
...
function findIndexByProperty(arr, name) {
  const findName = name;
  if (Array.isArray(arr)) {
    const findIndex = arr.findIndex(({ name }) => name === findName);
    return findIndex;
  } else {
    return -1;
  }
...
function getSubArrayAndConvertToString(arr, startIdx) {
  if (Array.isArray(arr)) {
    const sliceArr = arr.slice(startIdx).join(" ");
...
function getPositiveArrayValues(arr) {
  if (Array.isArray(arr)) {
    const iterValues = arr.values();
    const arrValues = Array.from(iterValues);
    const filterFn = (value, index, array) => value > 0;
    const positiveArrayValues = arrValues.filter(filterFn);
...
function removeAndDouble(arr, startIdx) {
  if (Array.isArray(arr)) {
    const arrDelet = arr.splice(startIdx);
...
function sumAndPrint(arr) {
  if (Array.isArray(arr)) {
    ...
    const sumElements = arr.forEach((elem, index, array) => {
      console.log(elem);
    });
...
function flattenSquareAndFilter(arr, threshold) {
  if (Array.isArray(arr)) {
    const flatArray = arr.flat(50);
    const arrSquare = flatArray.map((el) => el ** 2);
    const filterFn = (value, index, array) => value > threshold;

    const filteredSquaredArray = arrSquare.filter(filterFn);
...
function getLastElementsSumAndPrint(arr, n) {
  if (Array.isArray(arr)) {
    ...
    const sumElements = lastElements.forEach((elem, index, array) => {
      console.log(elem);
    });
    return sum;
  } else {
...
function insertElementAt(arr, index, element) {
  if (Array.isArray(arr)) {
    const lengthsArray = arr.splice(index, 0, element);
    return arr;
  } else {
    return "Вхідний параметр має бути масивом";
  }
...
function findLongestWord(arr) {
  if (Array.isArray(arr)) {
    const longestWord = arr.reduce((accumulator, curentValue) => {
      //   console.log(curentValue, curentValue.length);
      return curentValue.length > accumulator.length
        ? curentValue
        : accumulator;
    });
    return longestWord;
...
function calculateTotalPrice(arr) {
  if (Array.isArray(arr)) {
    const totalPrice = arr.reduce((summ, item) => summ + item.price, 0);
    return totalPrice;
  } else {
    return "";
  }
